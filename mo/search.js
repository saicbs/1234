...
function subscribeDel(subId){
    UserCookie();
    var url = "http://"+domain_name+"interface.dmzj.com/api/subscribe/del";
...
var update_read_status = function(subid){
    url = 'http://'+domain_name+'interface.dmzj.com/api/subscribe/upread';
...
function headSerch(obj){
    $(obj).bind('input propertychange',function() {
        ...
        var url ="http://"+domain_name+"interface.dmzj.com/api/wap_search"
        messageBox.show();
        if (keyword != '') {
            T.ajaxJsonp(url,{"keywords":keyword}, function (data) {
                ...
                if(data.result==1000){
                    for(i=0;i<data.data.length;i++){
                        html += '<li><a href="/search/'+data.data[i].name+'.html">'+data.data[i].name+'</a></li>';
                    }
                }else{
                    html = '<li><a href="javascript:;">无搜索结果</a></li>';
                }
                messageBox.html(html);
                $('form[id=searchForm]').attr('action','/search/'+keyword+'.html');
            },function(){
                messageBox.hide();
            });
        }else{
...
function serchAction(){
    ...
    if(keyword!=""){
        location.href="/q/?q="+keyword+"&m=&f=_all&s=create_time_DESC&p=1"
    }else{
        alert("请输入关键词");
    }
}
